...
function Team(){
  ...
  let [newCert, setNewCert] = useState(false);
  ...
  function adminPermissions(){
    if (admin){
      setAdminActions([ 
        <Button key="user" variant="light" onClick={() => {setNewUser(true), setShowEditUser(true)}}>+ Add User</Button>, //, setNewUser(true)
  ...
  return (
    <div>
      <Navigation />
      <EditUser show={showEditUser} setShow={setShowEditUser} newUser={newUser} setNewUser={setNewUser} data={user}/>
      <h1>Team Page</h1>
      {/* <EditCerttification show={showEditCert} setShow={setShowEditCert} newCert={newCert} /> */}
      {adminActions}
